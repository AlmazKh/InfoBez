...
@Data
public class DESStore {

    private Map<Integer, String> keyRoundMap = new HashMap<>();
    private Map<Integer, String> textRoundMap = new HashMap<>();
    ...
    String permutation(int[] sequence, String input) {
        ...
        for (int i = 0; i < sequence.length; i++)
            output += input.charAt(sequence[i] - 1 >= input.length() ?
                    input.length() - 1 : sequence[i] - 1);
    ...
    String round(String input, String key, int num) {
        ...
        textRoundMap.put(num + 1, strToBinary(left + right));
    ...
    Result encrypt(String plainText, String key) {
        ...
        // 16 rounds
        for (int i = 0; i < 16; i++) {
            keyRoundMap.put(i + 1, strToBinary(keys[i]));
